# -*- coding: utf-8 -*-
"""
Created on Mon Jan 12 22:08:42 2026

@author: arun2
"""
import CoolProp.CoolProp as CP
import math
import cantera as ct
import numpy as np
import Non_Equilibrium_Tank_Model_ZK as ZK
from scipy.integrate import solve_ivp
import matplotlib.pyplot as plt

# %% Tank Model - System of ODEs
def tank_model_funct(t,z, # Main Variables
                     HEOS, threshold, E, CS_tank, c_dict, n_dict, L, A, g, k_vap, k_liq, universal_gas_constant, Cd, P_exit, injector_area): # Args
    print('ITER START')
# Unpacking initial data____________________________________________________________________________________________________
    pressure_tank = z[0]
    temperature_VAP = z[1]
    temperature_LIQ = z[2]
    mass_VAP = z[3]
    mass_LIQ = z[4]
    volume_VAP = z[5]
    volume_LIQ = z[6]
    density_VAP = z[7]
    density_LIQ = z[8]
    i_energy_VAP = z[9]
    i_energy_LIQ = z[10]
    
# Intermediate parameters___________________________________________________________________________________________________
    mass_flux_out = ZK.mass_dot_outlet_funct (HEOS, 
                                           Cd, 
                                           pressure_tank, 
                                           temperature_LIQ, 
                                           P_exit,
                                           mass_LIQ,
                                           i_energy_LIQ,
                                           density_LIQ)
    dmass_OUT = injector_area * mass_flux_out
    HEOS.update(CP.PQ_INPUTS, pressure_tank, 0)
    temperature_surf = HEOS.T()
    #print(temperature_surf,temperature_LIQ)
    
# Mass transfer rate for evaporation and condensation_______________________________________________________________________
    dheat_in_liq = ZK.Heat_dot_funct (HEOS, 
                                      c_dict["liquid N2O"], 
                                      n_dict["liquid N2O"], 
                                      volume_LIQ, 
                                      A, 
                                      g, 
                                      temperature_LIQ, 
                                      temperature_surf, 
                                      CS_tank,
                                      k_liq)*E
    
    dheat_in_vap = ZK.Heat_dot_funct (HEOS, 
                                      c_dict["vapour N2O"], 
                                      n_dict["vapour N2O"], 
                                      volume_VAP, 
                                      A, 
                                      g, 
                                      temperature_VAP, 
                                      temperature_surf, 
                                      CS_tank,
                                      k_vap)

# Mass transfer rate for evaporation and condensation_______________________________________________________________________

    dmass_EVAP = ZK.mass_dot_evap_funct (HEOS, 
                             c_dict, 
                             n_dict, 
                             L, 
                             A, 
                             g, 
                             pressure_tank, 
                             volume_VAP, 
                             volume_LIQ, 
                             temperature_VAP, 
                             temperature_LIQ, 
                             E,
                             CS_tank,
                             k_vap,
                             k_liq,
                             mass_VAP,
                             mass_LIQ,
                             i_energy_VAP,
                             i_energy_LIQ,
                             density_VAP,
                             density_LIQ)
    
    dmass_COND = ZK.mass_dot_cond_funct (HEOS,
                             pressure_tank, 
                             volume_VAP,
                             temperature_VAP, 
                             temperature_LIQ,
                             E,
                             CS_tank,
                             universal_gas_constant,
                             time_step,
                             mass_VAP,
                             i_energy_VAP,
                             density_VAP)

# Mass transfer rate between the liquid and vapor phases____________________________________________________________________
    dmass_VAP = ZK.mass_dot_VAP_funct (dmass_EVAP, 
                                       dmass_COND)
    dmass_LIQ = ZK.mass_dot_LIQ_funct (dmass_EVAP, 
                                       dmass_COND,
                                       dmass_OUT)

# Volume change rate for liquid and vapour phases___________________________________________________________________________
    dvolume_VAP = ZK.volume_dot_vap_funct (HEOS, 
                                         pressure_tank, 
                                         mass_VAP, 
                                         mass_LIQ, 
                                         dmass_VAP, 
                                         dmass_LIQ, 
                                         volume_VAP, 
                                         volume_LIQ, 
                                         temperature_VAP, 
                                         temperature_LIQ,
                                         dmass_EVAP, 
                                         dmass_COND,
                                         dmass_OUT,
                                         dheat_in_vap,
                                         dheat_in_liq,
                                         i_energy_VAP,
                                         i_energy_LIQ)
    
    dvolume_LIQ = ZK.volume_dot_liq_funct (dvolume_VAP)
    
# Density change rate for liquid and vapour phases__________________________________________________________________________
    drho_VAP = ZK.density_dot_funct(HEOS, 
                                    mass_VAP, 
                                    dmass_VAP, 
                                    volume_VAP, 
                                    dvolume_VAP)
    
    drho_LIQ = ZK.density_dot_funct(HEOS, 
                                    mass_LIQ, 
                                    dmass_LIQ, 
                                    volume_LIQ, 
                                    dvolume_LIQ)

# Energy transfer rate between the liquid and vapour phases_________________________________________________________________
    denergy_VAP = ZK.energy_dot_vap_funct (HEOS, 
                                           pressure_tank, 
                                           temperature_VAP, 
                                           temperature_LIQ, 
                                           dmass_EVAP, 
                                           dmass_COND, 
                                           dheat_in_vap, 
                                           dvolume_VAP,
                                           mass_VAP,
                                           mass_LIQ,
                                           i_energy_VAP,
                                           i_energy_LIQ,
                                           density_VAP,
                                           density_LIQ)
    
    denergy_LIQ = ZK.energy_dot_liq_funct (HEOS, 
                                           pressure_tank,
                                           temperature_VAP,
                                           temperature_LIQ,
                                           volume_LIQ,
                                           dmass_EVAP, 
                                           dmass_COND, 
                                           dmass_OUT, 
                                           dheat_in_liq,  
                                           dvolume_LIQ, 
                                           threshold,
                                           mass_VAP,
                                           mass_LIQ,
                                           i_energy_VAP,
                                           i_energy_LIQ,
                                           density_VAP,
                                           density_LIQ)
    
# Temperature change rate for the liquid and vapor phases___________________________________________________________________
    dtemperature_VAP = ZK.temperature_dot_funct (HEOS, 
                                                 mass_VAP, 
                                                 dmass_VAP,
                                                 drho_VAP,
                                                 pressure_tank,
                                                 temperature_VAP,
                                                 denergy_VAP,
                                                 i_energy_VAP,
                                                 density_VAP)
    
    dtemperature_LIQ = ZK.temperature_dot_funct (HEOS, 
                                                 mass_LIQ, 
                                                 dmass_LIQ, 
                                                 drho_LIQ, 
                                                 pressure_tank, 
                                                 temperature_LIQ, 
                                                 denergy_LIQ,
                                                 i_energy_LIQ,
                                                 density_LIQ)

# Pressure change rate for tank_____________________________________________________________________________________________
    dpressure = ZK.pressure_dot_funct(HEOS,
                               mass_VAP,
                               mass_LIQ,
                               dmass_VAP,
                               dmass_LIQ,
                               temperature_VAP,
                               temperature_LIQ,
                               density_VAP,
                               density_LIQ,
                               i_energy_VAP,
                               i_energy_LIQ,
                               dtemperature_VAP, 
                               dtemperature_LIQ)

# Outputting combined system of ODE_________________________________________________________________________________________
    dZdt = np.zeros(len(z))
    dZdt[0] = dpressure
    dZdt[1] = dtemperature_VAP
    #dZdt[2] = dmass_COND
    #dZdt[3] = dmass_EVAP
    dZdt[2] = dtemperature_LIQ
    dZdt[3] = dmass_VAP
    dZdt[4] = dmass_LIQ
    #dZdt[9] = drho_LIQ
    #dZdt[10] = drho_VAP
    dZdt[5] = dvolume_VAP
    dZdt[6] = dvolume_LIQ
    dZdt[7] = drho_VAP
    dZdt[8] = drho_LIQ
    dZdt[9] = denergy_LIQ
    dZdt[10] = denergy_VAP
    print(t,'s||',dtemperature_LIQ,'K/s||', T_liq + dtemperature_LIQ*t)
    plt.plot(t,T_liq + dtemperature_LIQ*t,'.')
    return dZdt

# %% Input Data for Testing
HEOS = CP.AbstractState("HEOS&BICUBIC",'NitrousOxide')
HEOS.update(CP.PT_INPUTS,56*101325, 298)
#print(HEOS.T())
# Basic Constants
g = 9.81
universal_gas_constant = 8.314

# Tank Geometry
L_tank = 1
R_tank = 0.02
CS_tank = math.pi*(R_tank**2)

V_tank = L_tank*CS_tank

# Initial Tank conditions
T_vap = 298.15
T_liq = 298.15
P_tank_initial = 56*ct.one_atm

# Initial tank data
fill_percentage = 90/100
L_vap = L_tank*(1 - fill_percentage)
L_liq = L_tank*(fill_percentage)
V_vap = L_vap*CS_tank
V_liq = L_liq*CS_tank

HEOS.update(CP.PQ_INPUTS,P_tank_initial,1)
rho_vap = HEOS.rhomass()
m_vap = HEOS.rhomass()*V_vap
u_vap = HEOS.umass()

HEOS.specify_phase(CP.iphase_liquid)
HEOS.update(CP.PT_INPUTS,P_tank_initial,T_liq)
rho_liq = HEOS.rhomass()
m_liq = HEOS.rhomass()*V_liq
u_liq = HEOS.umass()

# Heat Transfer Constants
c_dict = {
    'liquid N2O': 0.15,
    'vapour N2O': 0.15
    }
n_dict = {
    'liquid N2O': 1/3,
    'vapour N2O': 1/3
    }
k_vap = 0.0173
k_liq = 0.15

# Solver Inputs
time_step = 0.01
threshold = 0.01
E = 693

# Injector and outflow data
Cd = 0.55
P_exit = 101325
injector_area = 2.54/1000000 

U_vap = u_vap*m_vap
U_liq = u_liq*m_liq
# %% Solving System of ODE
ti = 0
tf = 5
step_number = 1000
time_step = tf/step_number
t_eval = np.linspace(ti,tf,step_number)
y0 = np.zeros(11)
y0[0] = P_tank_initial
y0[1] = T_vap
y0[2] = T_liq
y0[3] = m_vap
y0[4] = m_liq
y0[5] = V_vap
y0[6] = V_liq
y0[7] = rho_vap
y0[8] = rho_liq
y0[9] = U_vap
y0[10] = U_liq

sol = solve_ivp(tank_model_funct,[ti,tf],y0,
                method='LSODA',
                t_eval=t_eval, 
                args=(HEOS, threshold, E, CS_tank, c_dict, n_dict, L_tank, CS_tank, g, k_vap, k_liq, universal_gas_constant, Cd, P_exit, injector_area),
                max_step=0.01)

# %% Plots


plt.plot(sol['t'],sol['y'][0])
plt.xlabel('Time, s')
plt.ylabel('Liquid Mass, Kg')
plt.show()

plt.plot(sol['t'],sol['y'][1])
plt.xlabel('Time, s')
plt.ylabel('Vapour Mass, Kg')
plt.show()

plt.plot(sol['t'],sol['y'][2])
plt.xlabel('Time, s')
plt.ylabel('Condensation Mass, Kg')
plt.show()

plt.plot(sol['t'],sol['y'][3])
plt.xlabel('Time, s')
plt.ylabel('Evaporation Mass, Kg')
plt.show()

plt.plot(sol['t'],sol['y'][4])
plt.xlabel('Time, s')
plt.ylabel('Exit Mass, Kg')
plt.show()

plt.plot(sol['t'],sol['y'][5])
plt.xlabel('Time, s')
plt.ylabel('Liquid Temperature, m^3')

plt.plot(sol['t'],sol['y'][6])
plt.xlabel('Time, s')
plt.ylabel('Vapour Temperature, m^3')
plt.show()

plt.plot(sol['t'],sol['y'][7])
plt.xlabel('Time, s')
plt.ylabel('Liquid Energy, J')
plt.show()

plt.plot(sol['t'],sol['y'][8])
plt.xlabel('Time, s')
plt.ylabel('Vapour Energy, J')
plt.show()

plt.plot(sol['t'],sol['y'][9])
plt.xlabel('Time, s')
plt.ylabel('Liquid Density, Kg/m^3')
plt.show()

plt.plot(sol['t'],sol['y'][10])
plt.xlabel('Time, s')
plt.ylabel('Vapour Density, Kg/m^3')
plt.show()

plt.plot(sol['t'],sol['y'][11])
plt.xlabel('Time, s')
plt.ylabel('Liquid Volumme, m^3')
plt.show()

plt.plot(sol['t'],sol['y'][12])
plt.xlabel('Time, s')
plt.ylabel('Vapour Volume, m^3')
plt.show()